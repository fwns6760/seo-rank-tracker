...
- 2026-03-17: `app/error.tsx`, `app/global-error.tsx`, `components/app-error-fallback.tsx` を追加し、server-side 例外時でも retry と主要画面への退避が可能な運用向けエラーバウンダリを整備した。
- 2026-03-17: `components/app-loading-state.tsx` と `app/loading.tsx` を追加し、BigQuery 集計待ちでも skeleton が表示されるようにした。`docs/project-conventions.md` に loading fallback 方針も追記した。
- 2026-03-17: `app/not-found.tsx` を追加し、404 時でも Dashboard / Pages / Settings へ戻れる運用向け fallback を整備した。
- 2026-03-17: `components/app-navigation.tsx` を追加し、現在地の route をヘッダーナビで強調表示するようにした。`components/app-shell.tsx` は nav を client component へ分離した。
