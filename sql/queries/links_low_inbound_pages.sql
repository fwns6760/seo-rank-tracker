...
WHERE (SELECT latest_crawl_date FROM latest_crawl) IS NOT NULL
  AND COALESCE(inbound_counts.incoming_links, 0) BETWEEN 1 AND 2
ORDER BY incoming_links ASC, tracked_pages.total_impressions DESC, tracked_pages.average_position ASC
LIMIT @limit;
